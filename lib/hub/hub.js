...
class Hub {
  ...
  *run() {
    ...
    this.defaults = this.manifest.defaults || {}

    this.services = _.map(this.manifest.services,
      (manifest, name) => {
        var mergedManifest = _.defaults(manifest, this.defaults)
        return this.createService(mergedManifest, name)
      }
    )
  ...
  *purge() {
    yield this.middlewares.map(_ => {
      log.trace({ name: _.instanceName }, 'Purging')
      return _.purge()
    })
  }
  ...
  *stop() {
    yield this.middlewares.map(_ => {
      log.trace({ name: _.instanceName }, 'Stopping')
      return _.stop()
    })
    this.middlewares = []
    this.registry = {}
  ...
  getBulkSubscribersOf(msg) {
    return this.services.filter(_ => _.isSubscriberOfBulk(msg))
  }
}
