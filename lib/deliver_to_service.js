...
module.exports = function(svc) {
  ...
  function *handleError(msg, err) {
    log.error({message: msg}, "Failed handling message")
    msg.attemptsMade += 1
  ...
  return function*(msg) {
    ...
    try {
      ...
      if (msg.attemptsMade > 0) {
        stats.histogram([
          ...
          'hub.' + svc.name + '.messages.${msg.type}.retries'
        ], msg.attemptsMade)
      }
    ...
    stats.timing(["hub.messages." + svc.name + "." + msg.type + ".response_time"], endedAt - startedAt)
  }
}
