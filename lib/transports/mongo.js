...
module.exports = function(url) {
  ...
  var deadCollection = (svc) => _this.db.collection(svc.name + "_dead")
  var scheduleCollection = (svc) => _this.db.collection(svc.name + "_schedule")

  this.destroyAll = function*(svc) {
    try{
      yield [
        deadCollection(svc).dropAsync(),
        scheduleCollection(svc).dropAsync()
      ]
    }
    catch (ex) {
  ...
  this.schedule = function*(service, msg, dueTime) {
    yield scheduleCollection(service).insertAsync({
      dueTime: dueTime,
      message: msg
    })
  }

  this.getDueMessages = function*(service, dueTime) {
    return yield scheduleCollection(service).findAsync({
      dueTime: {$lt: dueTime}
    })
  }

  this.removeScheduledMessage = function*(service, scheduledMsg) {
    yield scheduleCollection(service).removeAsync({
  ...
  this.getDeadMessages = function*(service, page, pageSize) {
    ...
    var messages = _.each(msgsAndStats[0], x => delete x._id)
