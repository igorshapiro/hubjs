...
class ServiceTransportDelegates {
  *enqueueOutgoing(msg) {
...
class Service extends ServiceTransportDelegates {
  ...
  *handleMessages(queueType, handler) {
    var response = yield this.channel.consume(this.buildEntityName(queueType), qmsg => {
      if (!qmsg) return
  ...
  *processQueues() {
    ...
    this.inConsumerTag = yield this.handleMessages('in', _ => this.deliverToService(_))
    this.outConsumerTag = yield this.handleMessages('out', _ => this.enqueueToServices(_))
  }
