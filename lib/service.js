...
module.exports = function(repo, manifest) {
  ...
  var buildEntityName = (type) => _this.name + "_" + type

  this.isSubscriberOf = (type) => _.includes(this.subscribes, type)
  this.isPublisherOf = (type) => _.includes(this.publishes, type)
  ...
  this.schedule = function*(msg, dueTime) {
  ...
  this.deleteDeadMessage = function*(msgId) {
    return yield this.storageTransport.deleteDeadMessage(this, msgId)
  }
  this.reenqueueMessage = function*(msgId) {
    ...
    var response = yield _this.channel.consume(buildEntityName(queueType), function(qmsg) {
      ...
      co(function*() {
        try {
          ...
          log.error({ err: ex }, "Message handler failed")
        }
        finally {
  ...
  function *startTransport() {
    ...
    _this.scheduler = new Scheduler(_this, repo.options)
    yield _this.scheduler.start()
  }
