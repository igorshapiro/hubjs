...
class Recurring extends Mongo {
  ...
  *register(msg) {
    ...
    var existingMsg = yield this.collection.findOne({_id: msg.messageType})
    if (existingMsg) {
      return yield this.collection.updateOne({_id: msg.messageType}, {
        message: msg
      })
    }
    yield this.collection.insertOne(record)
  }

  *unregister(recurringMessageId) {
    yield this.collection.deleteOne({'recurringMessageId': recurringMessageId})
  }
  ...
  *intervalHandler() { yield this.schedulePending() }
  *schedulePending() {
