...
class Scheduler extends MongoMiddleware {
  ...
  *delete(scheduledMessageId) {
    yield this.collection.deleteOne({'scheduledMessageId': scheduledMessageId})
  }

  *scheduleNow(msg) {
    if (typeof msg === 'string') {
      msg = yield this.collection.findOne({ scheduledMessageId: msg })
    }
    yield this.inQueue.enqueue(msg.message)
    yield this.collection.deleteOne({_id: msg._id})
    this.emit('enqueued', {
  ...
  *schedule(msg, dueTime) {
    ...
    yield this.collection.insert(scheduledItem)
    this.emit('scheduled', {
      msg: msg,
      dueTime: dueTime,
